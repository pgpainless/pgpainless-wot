...
class WebOfTrustCLI : Callable<Int> {
    ...
    @Option(
        names = ["--trust-root", "-r"],
        description = ["One or more certificates to use as trust-roots."],
        converter = [TrustRootsConverter::class],
        paramLabel = "FINGERPRINT")
    var optTrustRoot: List<TrustRoot> = listOf()

    @ArgGroup(exclusive = true) var optTrustAmount: TrustAmount = TrustAmount()

    @ArgGroup(exclusive = true, multiplicity = "1") lateinit var optKeyRing: CertificateSource
    ...
    */
    ...
    @Option(
        names = ["--gpg-ownertrust"], description = ["Read trust-roots from GnuPGs ownertrust."])
    var optGpgOwnerTrust = false

    @Option(
        names = ["--certification-network"],
        description =
            [
                "Treat the web of trust as a certification network instead of an authentication network."])
    var optCertificationNetwork = false

    @Option(
        names = ["--gossip"],
        description =
            ["Find arbitrary paths by treating all certificates as trust-roots with zero trust."])
    var optGossip = false
    ...
    @Option(
        names = ["--known-notation"],
        description = ["Add a notation to the list of known notations."],
        paramLabel = "NOTATION NAME")
    var optKnownNotations: Array<String> = arrayOf()

    class TrustAmount {
        @Option(
            names = ["--trust-amount", "-a"],
            description = ["The required amount of trust."],
            paramLabel = "AMOUNT")
        ...
        fun get(certificationNetwork: Boolean): Int {
            return when {
                ...
                optDouble -> 240 // --double
                    ...
                    if (certificationNetwork) 1200
                    else 120 // default 120, if --certification-network -> 1200
            }
        }
    }
    ...
    class CertificateSource {
        @Option(
            names = ["--keyring", "-k"],
            description = ["Specify a keyring file."],
            required = true,
            paramLabel = "FILE")
        ...
        @Option(
            ...
            description =
                [
                    "Specify a pgp-cert-d base directory. Leave empty to fallback to the default pgp-cert-d location."],
            arity = "0..1",
            fallbackValue = "",
            paramLabel = "PATH")
        ...
        val get: PGPCertificateStore
            get() {
                ...
                if (optPgpCertD == "") {
                    ...
                    val certD = PGPainlessCertD.fileBased(certDFile, InMemorySubkeyLookupFactory())
                    return PGPCertificateStoreAdapter(certD)
                }
                val certD =
                    PGPainlessCertD.fileBased(File(optPgpCertD!!), InMemorySubkeyLookupFactory())
                return PGPCertificateStoreAdapter(certD)
            }
    }
    ...
    val api: WebOfTrustAPI
        get() {
            val network =
                PGPNetworkParser(optKeyRing.get).buildNetwork(referenceTime = optReferenceTime)
    ...
    companion object {
        ...
        @JvmStatic fun main(args: Array<String>): Unit = exitProcess(execute(args))
        ...
        @JvmStatic val gpgHelper = GpgHelper("/usr/bin/gpg")
    }

    override fun toString(): String {
        val source =
            if (optKeyRing.optGpg) {
                "gpg"
            } else {
