...
class SQWOTFormatter : Formatter {
    ...
    override fun format(binding: Binding, amountMin: Int, amountReference: Int): String {
        ...
        return buildString {
            ...
            // [✓] 7F9116FEA90A5983936C7CFAA027DB2F3E1E118A Paul Schaub <vanitasvitae@fsfe.org>:
            // fully authenticated (100%)
            append(checkmark)
            appendLine(
                "${binding.fingerprint} ${binding.userId}: $authLevel authenticated (${percentage}%)")

            for ((pIndex, path) in pathList.withIndex()) {
                if (!singlePath) {
                    appendLine(
                        "  Path #${pIndex + 1} of ${pathList.size}, trust amount ${path.amount}:")
                }
                val originUserId =
                    if (path.root.userIds.isEmpty()) ""
                    else if (path.root.fingerprint == path.target.fingerprint)
                        " \"${path.root.userIds.keys.first()}\""
                    else " (\"${path.root.userIds.keys.first()}\")"
                //   ◯ AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA ("Alice")
                append(indent)
                appendLine("◯ ${path.root.fingerprint}$originUserId")

                for ((eIndex, edge) in path.certifications.withIndex()) {
                    val targetUserId =
                        if (edge !is Edge.Certification) ""
                        else if (eIndex == path.certifications.lastIndex) " \"${edge.userId}\""
                        else " (\"${edge.userId}\")"
                    append(indent)
                    appendLine(
                        "│   ${certDegree(edge.trustAmount)} the following " +
                            (if (eIndex == path.certifications.lastIndex) "binding"
                            else "certificate") +
                            " on ${dateFormat.format(edge.creationTime)}" +
                            (if (edge.expirationTime == null) ""
                            else " (expiry: ${dateFormat.format(edge.expirationTime)})") +
                            introducerType(edge))

                    append(indent)
                    append(if (eIndex != path.certifications.lastIndex) "├ " else "└ ")
