...
class CertificationNetworkTest : TestCase(CertificationNetworkVectors()) {
    ...
    fun `without --certification-network Alice can only authenticate Bob`(
        callback: ExecutionCallback
    ) {
        val bobArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 authenticate ${v.bobFpr} ${v.bobUid}"
                .split(" ")
                .toTypedArray()
        val carolArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 authenticate ${v.carolFpr} ${v.carolUid}"
                .split(" ")
                .toTypedArray()
        val daveArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 authenticate ${v.daveFpr} ${v.daveUid}"
                .split(" ")
                .toTypedArray()

        val bobOutput =
            """[✓] A68DF00EB82F9C49C27CC7723C5F5BBE6B790C05 <bob@example.org>: fully authenticated (100%)
...
"""
    ...
    fun `with --certification-network Alice can authenticate all`(callback: ExecutionCallback) {
        val bobArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 --certification-network authenticate ${v.bobFpr} ${v.bobUid}"
                .split(" ")
                .toTypedArray()
        val carolArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 --certification-network authenticate ${v.carolFpr} ${v.carolUid}"
                .split(" ")
                .toTypedArray()
        val daveArgs =
            "-k ${keyRingPath()} -r ${v.aliceFpr} -a 120 --certification-network authenticate ${v.daveFpr} ${v.daveUid}"
                .split(" ")
                .toTypedArray()
...
"""
        val carolOutput =
            """[✓] AB9EF1C89631519842ED559697557DD147D99C97 <carol@example.org>: fully authenticated (100%)
  ◯ B2B371214EF71AFD16E42C62D81360B4C0489225 ("<alice@example.org>")
...
"""
        val daveOutput =
            """[✓] 9A1AE937B5CB8BC46048AB63023CC01973ED9DF3 <dave@example.org>: fully authenticated (100%)
  ◯ B2B371214EF71AFD16E42C62D81360B4C0489225 ("<alice@example.org>")
  │   certified the following certificate on 2023-01-19
  ├ A68DF00EB82F9C49C27CC7723C5F5BBE6B790C05 ("<bob@example.org>")
  │   certified the following certificate on 2023-01-19
