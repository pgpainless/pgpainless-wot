...
/**
 * A [Path] comprises a root [Node], a list of edges ([Edge.Components][Edge.Component]), as well as
 * a residual depth.
 *
...
class Path(
    ...
    /**
     * Construct a [Path] only consisting of the trust root. The [Path] will have an empty list of
     * edges and an unconstrained residual [TrustDepth].
     *
     * @param root trust root
     */
    constructor(root: Node) : this(root, mutableListOf<Edge.Component>(), TrustDepth.unlimited())

    /**
     * Current target of the path. This corresponds to the target of the last entry in the edge
     * list.
     */
    val target: Node
        get() {
    ...
    /**
     * List of [Nodes][Node] (nodes) of the path. The first entry is the [root]. The other entries
     * are the targets of the edges.
     */
    ...
    /**
     * The length of the path, counted in nodes. A path with a single edge between node A and B has
     * length 2, the empty path with only a trust root has length 1.
     */
    val length: Int
        get() = edges.size + 1
    ...
    /**
     * Trust amount of the path. This corresponds to the smallest trust amount of any edge in the
     * path.
     */
    val amount: Int
        get() =
     ...
     *
     * @throws IllegalArgumentException if the target at the end of the path is not equal to the
     *   issuer of the edge.
     * @throws IllegalArgumentException if the path runs out of residual depth
     * @throws IllegalArgumentException if the addition of the [Edge.Component] would result in a
     *   cyclic path
     */
    fun append(nComponent: Edge.Component, certificationNetwork: Boolean) {
        ...
        require(certificationNetwork || residualDepth > 0) { "Not enough depth." }
        ...
        if ((edges.size == 0) &&
            (root.fingerprint == nComponent.target.fingerprint) &&
            (nComponent is Edge.Delegation)) {
            cyclic = true
        }
