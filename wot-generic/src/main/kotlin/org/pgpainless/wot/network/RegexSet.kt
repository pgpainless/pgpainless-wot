...
data class RegexSet(val regexStrings: Set<String>) {
    ...
    /**
     * Return true if the given [string] matches the [RegexSet]. That is if at least one RegEx in
     * the set matches the [string], or if the set represents a wildcard.
     */
    fun matches(string: String): Boolean {
        // wildcard or any match
        return regexStrings.isEmpty() ||
            regexStrings.any { Pattern.compile(it).matcher(string).find() }
    }
