...
/**
 * Edge between two nodes. The edge is made up of at least one delegation or certification, but can
...
class Edge(
    ...
    constructor(issuer: Node, target: Node) : this(issuer, target, mutableSetOf(), mutableMapOf())
    ...
    constructor(
        ...
        component.issuer,
        component.target,
        buildSet<Delegation> { if (component is Delegation) add(component) }.toMutableSet(),
    ...
    fun join(other: Edge): Edge {
        ...
        return this.apply {
            other.delegations.forEach { addComponent(it) }
            other.certifications.forEach { e -> e.value.forEach { addComponent(it) } }
        }
    }
    ...
    /**
     * Return a joined map of all components of this edge. Certifications are keyed by userId, while
     * delegations are keyed with null.
     */
    fun components(): Map<String?, List<Component>> {
        return mutableMapOf<String?, List<Component>>().apply {
            certifications.forEach { (uid, certs) -> put(uid, certs.map { it }) }
    ...
    override fun toString(): String {
        return buildString { components().values.flatten().forEach { appendLine(it.toString()) } }
    }
    ...
    class Certification(
        ...
        Component(
            ...
            exportable,
            trustAmount ?: 120,
            trustDepth ?: TrustDepth.limited(0),
            regexes ?: RegexSet.wildcard()) {
