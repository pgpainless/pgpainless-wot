...
class Dijkstra(private val rawNetwork: Network,
               private val roots: Set<TrustRoot>,
               private val certificationNetwork: Boolean = false,
               private val referenceTime: Date = Date()): ShortestPathAlgorithm {
     ...
     */
    override fun search(targetFpr: Identifier, targetUserid: String, targetTrustAmount: Int): Paths {
        ...
        if (!certificationNetwork) {
            ...
            roots.filter { it.trustAmount != FULLY_TRUSTED }
        ...
        while (paths.amount < targetTrustAmount) {
            ...
            val bestPath = roots.let { it.map { r -> r.fingerprint } }
    ...
    fun backwardPropagate(targetFpr: Identifier, targetUserid: String): HashMap<Identifier, Pair<Path, Int>> {
        val network = WotNetwork(rawNetwork, false, referenceTime) // these tests want authentication networks
     ...
     * are returned.
     */
    private fun backwardPropagate(network: WotNetwork, targetFpr: Identifier, targetUserid: String):
        ...
        while (true) {
            ...
            for (ec in ecs) {
                ...
                if (signeeFpr == targetFpr && ec is Edge.Certification && ec.userId != targetUserid
                ...
                val altCost = if (ec is Edge.Certification && ec.userId == targetUserid) {
    ...
    private fun reconstructPaths(network: WotNetwork, targetUserid: String, bestNextNode: HashMap<Identifier, ForwardPointer>, target: Node, dist: HashMap<Identifier, Cost>): HashMap<Identifier, Pair<Path, Int>> {
    ...
    private fun assemblePath(target: Node, targetUserid: String,
                             issuer: Node, bestNextNode: HashMap<Identifier, ForwardPointer>): Path {
