...
class Dijkstra(
    private val rawNetwork: Network,
    private val roots: Set<TrustRoot>,
    private val certificationNetwork: Boolean = false,
    private val referenceTime: Date = Date()
) : ShortestPathAlgorithm {

    private val logger: Logger = LoggerFactory.getLogger(Dijkstra::class.java)
    ...
    override fun search(
        ...
        targetUserid: String,
        targetTrustAmount: Int
    ): Paths {
        logger.debug("Authenticating <{}, '{}'>\nRoots: {}", targetFpr, targetUserid, roots)

        // Wrap the raw Network in a WotNetwork
        val network = WotNetwork(rawNetwork, certificationNetwork, referenceTime)
        // FIXME: add roots to the WotNetwork? -> handle their trust amounts from there (-> drop
        // suppressIssuer?)
        ...
        if (!certificationNetwork) {
            ...
            roots
                .filter { it.trustAmount != FULLY_TRUSTED }
                .forEach { network.capCertificate(it.fingerprint, it.trustAmount) }

            // FIXME: If A is a fully trusted root, and B is a root at trust amount 40, should B's
            // amount in
            ...
            // (Maybe the order in which paths are usually found will shadow this problem most of
            // the time?)
        }

        val paths = Paths()

        // Perform a (partial, until the targetTrustAmount is reached) run of the Ford-Fulkerson
        // algorithm
        // (https://en.wikipedia.org/wiki/Ford%E2%80%93Fulkerson_algorithm):
        //
        // Find a path, subtract that path from the network, then loop and search for more paths, if
        // any.
        while (paths.amount < targetTrustAmount) {
            val authPaths = backwardPropagate(network, targetFpr, targetUserid)

            // Pick one of the paths returned by backwardPropagate(), first by trust amount, then by
            // length.
            // We subtract that path from the network and search again, if we haven't yet reached
            // 'targetTrustAmount'.
            val bestPath =
                roots
                    .let { it.map { r -> r.fingerprint } }
                    ...
                    .maxWithOrNull(
                        compareBy(
                            ...
                            } // Break ties based on the fingerprint of the root.
                            ))
            ...
            if (bestPath != null) {
                ...
                // We made no progress in this iteration, there are no more paths to be found. We're
                // done.
                break
            }
        }
    ...
    fun backwardPropagate(
        targetFpr: Identifier,
        targetUserid: String
    ): HashMap<Identifier, Pair<Path, Int>> {
        val network =
            WotNetwork(rawNetwork, false, referenceTime) // these tests want authentication networks
        return backwardPropagate(network, targetFpr, targetUserid)
    }

    /**
     * Finds a path in the network from one or multiple `roots` that authenticates the target
     * binding.
     *
     * If `roots` is empty, authenticated paths starting from any node are returned.
     */
    private fun backwardPropagate(
        network: WotNetwork,
        targetFpr: Identifier,
        targetUserid: String
    ): HashMap<Identifier, Pair<Path, Int>> {
        logger.debug(
            "Query.backwardPropagate <{}, '{}'>\nRoots: {}", targetFpr, targetUserid, roots)
        ...
        // Note: The first step in the (reverse) paths we consider is always a certification of a
        // User ID binding.
        // All other steps must be delegations with the minimum depth appropriate to the path's
        // length.

        val prev: HashMap<Identifier, ForwardPointer> = HashMap()
        val dist: HashMap<Identifier, Cost> = HashMap()

        val queue: PairPriorityQueue<Identifier, Cost> = PairPriorityQueue()
        ...
        if (selfSig != null && selfSigAmount > 0) {
            ...
            // Arriving indirectly to the targetNode's User ID, via a delegation plus a self-signed
            // binding
            ...
            // The first step of the (backwards-facing) path:
            // We will arrive directly at the targetNode's relevant User ID, via a third party
            // certification
        ...
        while (true) {
            // To be safe, we're not using the cost from the priority queue (which is available in
            // `pop().second`).
            ...
            val root = roots.find { it.fingerprint == signeeFpr }
            if ((root != null) && (root.trustAmount >= FULLY_TRUSTED)) {
                logger.debug("  Skipping signee that is a fully trusted root")
                continue
            }

            val signee =
                network.nodeByFpr(signeeFpr)!! // We expect that the signee exists in the Network

            // Get the signee's current forward pointer (the edge that currently points at the
            // signee)
            ...
            // (with the necessary minimum depth, and matching the targetUserid to the certification
            // regex, if any)

            // We are considering two different possibilities: extending the existing path, or
            // effectively
            // *replacing* the existing path. The latter happens if our path currently consists of a
            // self-sig,
            // but we're "replacing" that path with a third party sig.

            // XX: ask for "depth=n-1" to be able to replace a terminating self-sig with a depth 0
            // third party sig.
            ...
            for (ec in ecs) {
                ...
                if (signeeFpr == targetFpr &&
                    ec is Edge.Certification &&
                    ec.userId != targetUserid
                ...
                val altCost =
                    if (ec is Edge.Certification && ec.userId == targetUserid) {
                        // This path replaces a direct signature
                        Cost(1, amount)
                    } else {
                        // XX: temp hack, see above
                        if (!certificationNetwork && (ec.trustDepth ?: 0) < signeeCost.length) {
                            logger.debug(
                                "      Certification does not allow enough depth ({}, needed: {}), skipping",
                                ec.trustDepth,
                                signeeCost.length)
                            continue
                        }
                ...
                // If we haven't visited this node before, or the new cost is preferable, store or
                // update pointer+cost
    ...
    private fun reconstructPaths(
        network: WotNetwork,
        targetUserid: String,
        bestNextNode: HashMap<Identifier, ForwardPointer>,
        target: Node,
        dist: HashMap<Identifier, Cost>
    ): HashMap<Identifier, Pair<Path, Int>> {
        // Follow the forward pointers and reconstruct the paths.
        val paths: HashMap<Identifier, Pair<Path, Int>> = HashMap()

        bestNextNode.entries
            // If roots were specified, only reconstruct paths for roots
            .filter { roots.isEmpty() || roots.any { root -> root.fingerprint == it.key } }
    ...
    private fun assemblePath(
        ...
        while (true) {
            ...
            if (ec is Edge.Certification && ec.userId == targetUserid) {
                // We've arrived (the target node may have an extra self-sig forward pointer to
                // itself, but we don't
