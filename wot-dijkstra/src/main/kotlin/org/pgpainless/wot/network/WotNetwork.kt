 ...
 * - Expose an (experimental) API that simplifies the implementation of the WoT algorithm (in
 *   `Query`)
 *     - TODO: polish, and possibly upstream into `Network`
 * - Form residual networks for the Ford-Fulkerson algorithm via `suppressPath()`
 *     - `getEffectiveTrustAmount()` returns effective trust amounts of certifications (taking the
 *       residual network into account)
 */
internal class WotNetwork(
    ...
    fun getSelfSig(targetFpr: Identifier, targetUserid: String): Edge.Component? {
        ...
        if (target?.userIds?.get(targetUserid) != null) {
            ...
            return Edge.Certification(
                target, target, targetUserid, referenceTime, null, true, 120, TrustDepth.limited(0))
        } else return null
    }
     ...
     * (FIXME: filter by target User ID, if fpr == target, and target user id != null, then simplify
     * Query?)
     *
     * In authentication network mode:
     * - only return Edge.Components whose depth allows authentication of a path that is `curLen`
     *   long.
     * - only return Edge.Components whose regexes match `targetUserid`.
     */
    fun certificationsForSignee(
        fpr: Identifier,
        targetUserid: String,
        curLen: Int
    ): List<Edge.Component> {
        ...
        val ec =
            edges
                .map { edge -> edge.components().map { it.value }.flatten() }
                .flatten()
                .filter {
                    if (!certificationNetwork) {
                        ...
                        // Certification network mode: Keep certifications of any depth, and ignore
                        // regex scoping
                        true
                    }
                }
    ...
    fun isValidTarget(targetFpr: Identifier, targetUserid: String): Node? {
        ...
        if ((target.expirationTime != null) && (target.expirationTime!! <= referenceTime)) {
            return null
        }
    ...
    /**
     * Get effective trust amount for a certification: Cap if the root has limited trust, and take
     * into account additional constraints of the residual network.
     */
    ...
    /**
     * Add suppression rules for all certifications along the specified path: Each edge is
     * suppressed by `amountToSuppress`.
     */
